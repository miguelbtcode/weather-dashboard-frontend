...
export const OtherCities: React.FC = () => {
  const { savedCities, switchToCity, tempUnit } = useWeather();
    ...
    return (
      <div className="bg-secondary-800 rounded-2xl p-6 animate-fade-in">
        <div className="flex justify-between items-center mb-4">
          ...
          <button className="text-xs text-secondary-400 hover:text-white">Show All</button>
        </div>
        ...
        {savedCities.map((city, index) => (
          ...
          >
            ...
            <div className="flex items-center">
              ...
              <span className="ml-2 text-2xl font-medium">{Math.round(city.temp)}°</span>
            </div>
          </div>
        ))}
      </div>
